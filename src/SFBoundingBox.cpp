...
SFBoundingBox::SFBoundingBox(const Vector2 centre, const int width,
		const int height) :
		centre(make_shared < Vector2 > (centre)), extent_x(
				make_shared < Vector2 > (Vector2((float) width / 2.0, 0))), extent_y(
				make_shared < Vector2 > (Vector2(0, (float) height / 2.0))) {
}
...
pair<float, float> SFBoundingBox::projectOntoAxis(const SFBoundingBox & b,
		enum AXIS axis) {
	float lo, hi;
...
bool SFBoundingBox::CollidesWith(const shared_ptr<SFBoundingBox> b) {
	pair<float, float> a_x_proj = projectOntoAxis(*this, X), a_y_proj =
			projectOntoAxis(*this, Y), b_x_proj = projectOntoAxis(*b, X),
...
ostream& operator<<(ostream& os, const SFBoundingBox& obj) {
	os << "c:(" << obj.centre->getX() << ", " << obj.centre->getY() << ") w:"
			<< (obj.extent_x->getX() * 2) << " h:"
			<< (obj.extent_y->getY() * 2);
	return os;
}
