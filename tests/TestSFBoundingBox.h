...
class TestSFBoundingBox: public CPPUNIT_NS::TestCase {
	...
	CPPUNIT_TEST (testSmallLarge);CPPUNIT_TEST_SUITE_END();

public:
	TestSFBoundingBox() :
			CppUnit::TestCase("TestSFBoundingBox") {
	}
	TestSFBoundingBox(std::string name) :
	...
	void testOverlap() {
		auto b1 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(0.0f, 0.0f), 5.0f, 5.0f));
		auto b2 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(2.5f, 2.5f), 5.0f, 5.0f));
	...
	void testDisjoint() {
		auto b1 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(0.0f, 0.0f), 5.0f, 5.0f));
		auto b2 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(20.0f, 20.0f), 5.0f, 5.0f));
	...
	void testSmallLarge() {
		...
		auto b1 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(10.0f, 119.0f), 2.0f, 2.0f));
		auto b2 = make_shared < SFBoundingBox
				> (SFBoundingBox(Vector2(0.0f, 200.0f), 288.0f, 298.0f));
